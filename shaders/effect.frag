...
float cubeTexture(vec3 p){float stripes=sin(p.x*20.0)*0.1+sin(p.y*25.0)*0.1;float grain=noise(p.xy*5.0+iTime*0.1)*0.05;float scratches=sin(p.z*40.0+noise(p.xy*10.0)*10.0)*0.05;return stripes+grain+scratches;}

void main(){
    vec2 uv=(gl_FragCoord.xy*2.0-iResolution.xy)/iResolution.y;
    vec3 ro=vec3(0.0,0.0,-3.0-uBass*1.5);
    vec3 rd=normalize(vec3(uv,1.0));
    float t=0.0,d=0.0; vec3 finalPos=vec3(0.0);
    for(int i=0;i<80;i++){
        vec3 pos=ro+rd*t;
        vec3 p=rotateY(pos,iTime*0.2+uBass*5.0);
        p=rotateX(p,iTime*0.1+uBass*3.0);
        vec3 cubeSize=vec3(0.5+uBass*0.5);
        d=sdBox(p,cubeSize);
        t+=max(d,0.01); finalPos=p;
        if(t>10.0) break;
    }
    vec3 col=vec3(0.2,0.25,0.3)*(1.0-smoothstep(0.0,0.05,d));
    float tex=cubeTexture(finalPos*3.0); col-=tex; col+=uBass*0.6;
